...
if {!$::aocl_incremental::incremental_compile} {
  ...
  # Importing dump kernel to other pr regions
  project_open top -revision top
  if {[catch {design::import_block kernel_1 -file dump.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  if {[catch {design::import_block kernel_2 -file dump.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  if {[catch {design::import_block kernel_3 -file dump.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  project_close
