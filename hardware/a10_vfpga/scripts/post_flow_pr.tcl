...
if {$do_update_mif & !$fast_compile & [string match $revision_name "top"]} {
    # temporally don't update base
...
# in this version, adjust pll cause error --> intel problem
# source  adjust_plls_a10.tcl

# generate bitstream now
...
if {[string match $revision_name "base"]} {
  ...
  qexec "quartus_cdb -t scripts/create_fpga_bin_pr.tcl fpga.bin base.sof"
} elseif {[string match $revision_name "top"]} {
  ...
  qexec "quartus_cdb -t scripts/create_fpga_bin_pr.tcl fpga_pr.bin top.sof top.kernel.rbf pr_base.id quartus_version.id"
  qexec "quartus_cdb -t scripts/create_fpga_bin_pr.tcl fpga_pr_1.bin top.sof top.kernel_1.rbf pr_base.id quartus_version.id"
  qexec "quartus_cdb -t scripts/create_fpga_bin_pr.tcl fpga_pr_2.bin top.sof top.kernel_2.rbf pr_base.id quartus_version.id"
  qexec "quartus_cdb -t scripts/create_fpga_bin_pr.tcl fpga_pr_3.bin top.sof top.kernel_3.rbf pr_base.id quartus_version.id"
} elseif {[string match $revision_name "flat"]} {
  post_message "Compiling flat revision -> adding only flat.sof to fpga.bin!"
  qexec "quartus_cdb -t scripts/create_fpga_bin_pr.tcl fpga.bin flat.sof"
}
