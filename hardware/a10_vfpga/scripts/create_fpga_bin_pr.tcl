...
if { $argc != $num_expected_args && $argc != 1 } {
   post_message "$prog: Need either exactly two or five arguments: output file, a SOF file (and optional: a RBF file, a PR base revision ID file and a Quartus version file)"
   exit 2
}
...
set sof_file [ lindex $files 1 ]
if { $num_files == $num_expected_args - 1 } { 
  set rbf_file [ lindex $files 2 ]
  set pr_base_id_file [ lindex $files 3 ]
  set quartus_version_file [ lindex $files 4 ]
}
