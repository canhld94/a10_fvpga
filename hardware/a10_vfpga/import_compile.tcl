...
set top_path_1 "freeze_wrapper_inst_1|kernel_system_inst"
set top_path_2 "freeze_wrapper_inst_2|kernel_system_inst"
set top_path_3 "freeze_wrapper_inst_3|kernel_system_inst"

# set-up OpenCL BSP utilities
::opencl_bsp::pre_synth_init "top" "top" "base" $top_path
::opencl_bsp::pre_synth_init "top" "top" "base" $top_path_1
::opencl_bsp::pre_synth_init "top" "top" "base" $top_path_2
::opencl_bsp::pre_synth_init "top" "top" "base" $top_path_3
...
if {!$::aocl_incremental::incremental_compile} {
  ...
  # Importing the kernel from the top_synth compile to all PR regions
  project_open top -revision top
  if {[catch {design::import_block kernel -file kernel.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  if {[catch {design::import_block kernel_1 -file kernel.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  if {[catch {design::import_block kernel_2 -file kernel.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  if {[catch {design::import_block kernel_3 -file kernel.qdb} result]} {
    post_message -type error "Error! $result"
    exit 2
  }  
  project_close
...
# ::aocl_incremental::fit_retry "top" "top"
# ::aocl_incremental::fit_retry "top" "top" $top_path_1
# ::aocl_incremental::fit_retry "top" "top" $top_path_2
# ::aocl_incremental::fit_retry "top" "top" $top_path_3
